...
def computeOccupancy_6BS(k_occ_whole, w_occ_whole, bs_ignore=[0, 5]):
    ...
    for t, (k_occ, w_occ) in enumerate(zip(k_occ_whole, w_occ_whole)):
        ...
        for i in range(len(occ)):
            if len(k_occ[i]) > 0:
                occ[i] = 'K'
            # K+ has a higher priority than water
            if len(w_occ[i]) > 0:
                if occ[i] == 'K':
...
@countTime
def run(coor, traj, distance=False):
    ...
    k_idx, cl_idx, water_idx = getSolventIndex(coor)
    ...
    occupancy[:len(k_occupancy)] = computeOccupancy_6BS(k_occupancy, w_occupancy)
    ...
    print(f"# K+: {len(k_idx)}")
    print(f"# water: {len(water_idx)}")
    print(f"# net water permeation = {n_w_netjumps}")
    print(f"# net ion permeation = {n_k_netjumps}\ncurrent = {current:.2f} pA")
    ...
    data_loc = os.path.join(path, 'data.csv')
    data = pd.DataFrame(data, columns=columns)
    _ = data.to_csv(data_loc)
    print(f"data saved to {data_loc}")
